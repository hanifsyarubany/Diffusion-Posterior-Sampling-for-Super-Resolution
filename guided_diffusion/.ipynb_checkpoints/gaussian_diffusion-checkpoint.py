...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            # Enable gradients w.r.t. x_t (needed for DPS/MCG guidance).
            img = img.requires_grad_()
            # Unconditional diffusion step p(x_{t-1} | x_t)
            out = self.p_sample(x=img, t=time, model=model)
            # Simulate noisy measurement at this timestep (for projection-type methods)
            noisy_measurement = self.q_sample(measurement, t=time)
            # Apply conditioning (DPS / MCG / etc.)
            # measurement_cond_fn is e.g. cond_method.conditioning(...)
            ...
            # Detach to avoid graph blow-up, free memory
            ######################
            img = img.detach_()
    ...
    def p_mean_variance(self, model, x, t):
        ...
        # If variance is learned, the network outputs 2*C channels:
        if model_output.shape[1] == 2 * x.shape[1]:
            model_output, model_var_values = torch.split(
                model_output, x.shape[1], dim=1
            )
        else:
            # Here model_var_values is only used to provide shape information to var_processor (for fixed variance cases).
            model_var_values = model_output
        # Interpret mean + x0 according to model_mean_type
        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(
            x, t, model_output
        )
        # Interpret variance according to model_var_type
...
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=0.0):
        ...
        # Eq. (12) in the DDIM paper: variance of the DDIM step
        sigma = (
            eta
